  ...
  return <Stack>
    ...
    <ol>
      ...
      <li>
        ...
        <span> Ostatnia wykryta aktywność na koncie - {dateForUI(currentUser?.session.last_used)}</span><br/>
        <span> Kiedy nastąpiło zalogowanie - {dateForUI(currentUser?.session.created_at)}</span><br/>
        <span> Kiedy wygasa - {dateForUI(currentUser?.session.expires_at)}</span><br/>
        ...
        <span>{userAgentForUI(currentUser?.session.user_agent)}</span><br/>
      ...
      {sessions.map((el) => {
        ...
        return <li key={dateForUI(el.created_at)}>
          <span><span style={{color: 'green'}}>Zalogowany</span></span><br/>
          <span> Ostatnia wykryta aktywność na koncie - {dateForUI(el.last_used)}</span><br/>
          <span> Kiedy nastąpiło zalogowanie - {dateForUI(el.created_at)}</span><br/>
          <span> Kiedy wygasa - {dateForUI(el.expires_at)}</span><br/>
          <span> Tryb sesji - {el.session_mode}</span><br/>
          <span> Kraj i język - {el.location} {el.language}</span><br/>
          <span>{userAgentForUI(el.user_agent)}</span><br/>
    ...
    <ol style={{
      ...
      {loginEventLogs.map((el) => <li
        key={dateForUI(el.created_at) + el.event_log_id}>
        <span>{dateForUI(el.created_at)} </span>
